...
mod tests {
    ...
    fn test_read_atom() {
        ...
        assert_eq!(atom, RujeAtom::Int(123));

        let mut reader = Reader::new("123.456");
        let atom = reader.read_atom().unwrap();
        assert_eq!(atom, RujeAtom::Float(123.456));

        let mut reader = Reader::new("abc");
        let atom = reader.read_atom().unwrap();
        assert_eq!(atom, RujeAtom::Symbol("abc".to_string()));
    }
}
